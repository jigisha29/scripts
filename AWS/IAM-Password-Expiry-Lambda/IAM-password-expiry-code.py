...
try:
    BLACKHOLE_GROUPNAME = os.environ['BLACKHOLE_GROUPNAME'] # group name where expire user will transfer
    ACTION_TOPIC_ARN = os.environ['ACTION_TOPIC_ARN'] # SNS topic for sending summary
    GRACE_PERIOD = int(os.environ['GRACE_PERIOD']) # days to send the user a warning email before the lockout occurs
    DISABLE_USERS = os.environ['DISABLE_USERS'] # Set to True if you want to disabled expired users - default - false
    SEND_EMAIL = os.environ['SEND_EMAIL'] # Set to True if SES is setup to allow you to email your user base
    FROM_ADDRESS = os.environ['FROM_ADDRESS'] # from email id with SES validation
    EXPLANATION_FOOTER = os.environ['EXPLANATION_FOOTER'] # If you have any questions or need assistance please reply to this email.
    EXPLANATION_HEADER = os.environ['EXPLANATION_HEADER'] # Company Policy requires passwords and access keys to be changed every 90 days.
...
def process_UsersCron(iam_client):
    ...
    max_age = 90
    # function not working max_age = get_max_password_age(iam_client)
    ...
    for row in credential_report:
        ...
        userid = "" #username of email ID of user
        useremail = "" # EMail id
        ...
        try:
            ...
            # process for getting user email ID
        #tags = iam_client.list_user_tags( UserName=row['user'] )
        #if tags["Key"] == 'ID':
        #    userid = tags["value"]
        try:
            jname = iam_client.get_user( UserName=row['user'])
            user = jname['User']
            if 'Tags' in user:
        ...
        if message != "":
            ...
            email_user(useremail, message, account_name)
